...
def enhancedFeatureExtractorDigit(datum):
    ...
    loop=False
    #searches for existing pixel to north, south, east, and west
    def search(x,y):
        if (x,y) in traveled.keys():
            return True
        else:
            if x < 0 or x >= width:
                return False
        ...
        traveled[(x,y)] = True
        return search(x - 1 , y) and search(x + 1 , y) and search(x , y - 1) and search(x , y + 1)

    for x_coord in range(width):
        for y_coord in range(height):
            traveled = {}
            if datum.getPixel(x_coord,y_coord) == 0:
                if search(x_coord,y_coord) == True:
                    loop=True
                    break
...
def enhancedPacmanFeatures(state, action):
    ...
    successor = state.generateSuccessor(0, action)
    #feature for each food
    for food in foodList:
        features[str(food)] = 1.0 / (util.manhattanDistance(food, successor.getPacmanPosition()) + 1)
    #feature for score
    features["score"] = successor.getScore()
    ghosts = successor.getGhostStates()
    #feature for amount of scared ghosts
    for ghost in ghosts:
