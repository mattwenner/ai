        ...
        *** GORY DETAIL AHEAD ***
        ...
        for position in self.legalPositions:
        	...
        	for newPos, prob in newPostDist.items():
                #find new beliefs
        		nBeliefs[newPos] += currBelief * prob
        #update beliefs
        self.beliefs = nBeliefs
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        #place particles
        while n > 0:
        	if n > len(legalPositions):
        		uniformList += legalPositions
        		n -=len(legalPositions)
        	else:
        		uniformList += legalPositions[0:n-1]
        		n = 0
        self.particleList = uniformList
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
        	...
        	for particle in self.particleList:
        		pDistance = util.manhattanDistance(particle, pacmanPosition)
                #make new beliefs
        		newCounter[particle] +=emissionModel[pDistance]
            #update beliefs
        	self.beliefs = newCounter
        #recreate beliefs
        if self.beliefs.totalCount() == 0:
            ...
            #update particle list with new particle positions
    ...
    def elapseTime(self, gameState):
        ...
        for particle in self.particleList:
            ...
            #create new particle list based on a sample created from the new position distribution
        	nParticles.append(util.sample(newPosDist))
        self.particleList = nParticles
    ...
    def getBeliefDistribution(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        #create belief distribution that reflects current partiblel positions
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        legalPositions = itertools.product(self.legalPositions, repeat = self.numGhosts)
        #array of legal positions
        legalArray = []
        for position in legalPositions:
        	legalArray.append(position)
        random.shuffle(legalArray)

        #designate particles to positions
        n = self.numParticles
        uniformList = []
        while n > 0:
        	if n > len(legalArray):
        		uniformList += legalArray
        		n -=len(legalArray)
        	else:
        		uniformList += legalArray[0:n-1]
        		n = 0
        self.particleList = uniformList
    ...
    def observeState(self, gameState):
        ...
        #resample particles
        for particle in self.particleList:
            #update all jailed ghosts' particles
        	for ghostNum in jailList:
        		particle = self.getParticleWithGhostInJail(particle, ghostNum)

        	probability = 1
        	for i in range(self.numGhosts):
        		if i not in jailList:
        			trueDistance = util.manhattanDistance(particle[i], pacmanPosition)
        			probability *=emissionModels[i][trueDistance]
        	newCounter[particle]+=probability
        ...
        if self.beliefs.totalCount() == 0:
        	...
        	for index in range(len(self.particleList)):
        		newParticlePositions = util.sample(self.beliefs)
        		newParticleList.append(newParticlePositions)
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particleList:
            ...
            prevGhostPos = list(oldParticle)
            "*** YOUR CODE HERE ***"
            for i in range(self.numGhosts):
            	newPosDist = getPositionDistributionForGhost(setGhostPositions(gameState, prevGhostPos), i, self.ghostAgents[i])
